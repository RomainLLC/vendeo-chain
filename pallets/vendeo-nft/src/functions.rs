...
pub const SALT_VENDEO_NFT: &[u8; 8] = b"VndoNft/";
...
where
	T: pallet_uniques::Config<ClassId = CollectionId, InstanceId = NftId>,
{
	/// Encodes a VENDEO NFT with randomness + `collection_id` + `nft_id` into a virtual account
	...
	pub fn nft_to_account_id<AccountId: Codec>(
		...
		(SALT_VENDEO_NFT, collection_id, nft_id)
			.using_encoded(|b| AccountId::decode(&mut TrailingZeroInput::new(b)))
			.expect("Decoding with trailing zero never fails; qed.")
	}

	/// Decodes a VENDEO NFT a suspected virtual account
	...
	pub fn decode_nft_account_id<AccountId: Codec>(
		...
		if &prefix == SALT_VENDEO_NFT && suffix.iter().all(|&x| x == 0) {
			Some(tuple)
		} else {
			None
		}
